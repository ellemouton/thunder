...
import (
	...
	"github.com/ellemouton/thunder/etcd"
)
...
func (c *client) Create(ctx context.Context, paymentHash lntypes.Hash, resourceType string, resourceID int64) (*macaroon.Macaroon, error) {
	var rootKey [32]byte
	...
	c.etcdClient.Put(ctx, idKey, string(rootKey[:]))
...
func (c *client) Verify(ctx context.Context, macBytes []byte, preimage []byte, resourceType string, resourceID int64) (bool, error) {
	...
	resp, err := c.etcdClient.Get(ctx, idKey)
	if err != nil {
		return false, err
	}
